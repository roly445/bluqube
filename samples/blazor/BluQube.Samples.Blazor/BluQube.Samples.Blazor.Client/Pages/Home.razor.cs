...
public partial class Home(ICommandRunner commandRunner, IQueryRunner queryRunner)
{
    ...
    {
        var result = await queryRunner.Send(new GetAllToDoItemsQuery());
    ...
    {
        var result = await commandRunner.Send(new AddTodoCommand(this._addToDoModel.Title));
    ...
    {
        var result = await commandRunner.Send(new DeleteTodoCommand(item.Id));
        if (result.Status == CommandResultStatus.Succeeded)
        {
    ...
    {
        var result = await commandRunner.Send(new UpdateToDoTitleCommand(item.Id, item.EditedTitle!));
        if (result.Status == CommandResultStatus.Succeeded)
        {
