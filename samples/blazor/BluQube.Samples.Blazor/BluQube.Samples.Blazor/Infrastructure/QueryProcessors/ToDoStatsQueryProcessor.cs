...
{
    public ValueTask<QueryResult<ToDoStatsQueryResult>> Handle(ToDoStatsQuery request, CancellationToken cancellationToken)
    {
        var result = new ToDoStatsQueryResult(
            todoService.Todos.Count(),
            todoService.Todos.Count(t => t.IsCompleted));

        return ValueTask.FromResult(QueryResult<ToDoStatsQueryResult>.Succeeded(result));
    }
}